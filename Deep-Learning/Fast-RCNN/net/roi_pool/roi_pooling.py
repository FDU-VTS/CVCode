...
# pooled_height/pooled_width: roi pooling size
# spatial_scale: the scale to convert input image to feature map
class RoIPoolingFunction(Function):
    ...
    def forward(self, features, rois):
        ...
        if not features.is_cuda:
            ...
            roi_pooling.roi_pooling_forward(self.pooled_height, self.pooled_width,
                                            self.spatial_scale, _features, rois, output)
        else:
            output = output.cuda()
            argmax = argmax.cuda()
