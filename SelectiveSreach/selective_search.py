...
def _generate_segments(im_orig, scale, sigma, min_size):
    ...
    # get image with (r,g,b,0)
    im_orig = np.append(
        im_orig, np.zeros(im_orig.shape[:2])[:, :, np.newaxis], axis=2)
    # get image with (r,g,b,mask_indice)
    im_orig[:, :, 3] = im_mask
...
def _extract_regions(img):
    ...
    # R:every region to be collected to 1113 size
    ...
    # use LBP to calculate the texture grad
...
# get neighbours tuple list like [(a,b),(a,c)],
# represent a and b have a overlap and a,c have a overlap
def _extract_neighbours(regions):

    # if a and b have a overlap
    def intersect(a, b):
...
if __name__ == "__main__":
    ...
    img_lbl, regions = selective_search(
        origin_image, scale=500, sigma=0.9, min_size=10)
